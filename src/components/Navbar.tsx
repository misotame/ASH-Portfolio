  ...
  return (
    <>
      <nav className={`fixed top-0 w-full z-[80] transition-all duration-500 
        ...
        ${isMenuOpen 
          ? 'bg-medical-dark' 
          : `bg-white/80 backdrop-blur-xl border-b border-white/20 shadow-sm ${isDarkPage ? 'lg:bg-transparent lg:backdrop-blur-none lg:border-none lg:shadow-none lg:text-white' : 'lg:bg-white/90 lg:backdrop-blur-md lg:border-b lg:border-medical-soft/20 lg:text-medical-dark'}`
        }`}>
        ...
        <Link to="/" className="flex items-center gap-2 z-10">
          ...
          <span className={`text-2xl font-bold tracking-tight transition-colors ${isMenuOpen ? 'text-white' : `text-medical-dark ${isDarkPage ? 'lg:text-white' : 'lg:text-medical-dark'}`}`}>Medify</span>
        </Link>
          ...
          {navItems.map((item) => (
            item.href.startsWith('#') && isHome ? (
              <a 
                key={item.name} 
                href={item.href} 
                className={`px-5 py-2 rounded-full text-sm font-semibold transition-all hover:bg-white/20 ${isDarkPage ? 'text-white' : 'text-medical-dark'}`}
              >
                {item.name}
              </a>
            ) : (
